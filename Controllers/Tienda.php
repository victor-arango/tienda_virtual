			...
			if($_POST){
				if(empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmailCliente'])  )
				{
					...
					if($request_user > 0 )
					{
						...
						$arrResponse = array('status' => false, 'msg' => '¡Atención! el email  ya existe. valida tu informacion ');		
					}else{
					...
					if(empty($_POST['datapay'])){
						...
						if(is_object($objPaypal)){
							...
							if($objPaypal->status == "COMPLETED"){
								...
								$arrResponse = array("status" => false, "msg" => 'No fue posible completar el pago con paypal');
							}
						}else{
